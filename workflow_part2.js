...
const batchResults = await pipeline(
  ...
  batch => agent(
    ...
    `   🗓 ОКНО 24 МЕСЯЦА: читаешь всю историю, но в бриф и заход кладёшь только касания за последние 2 года ` +
    `(ивенты, переписка, боль, договорённости). Старше — НЕ упоминаешь. Исключение — стоп-факторы ` +
    `(статус клиента/отток, финал won-сделки churn_*, конкурент, сожжённые кейсы, смена работодателя): ` +
    `их проверяешь на всю глубину и выносишь строкой «Старше 2 лет (не для захода): …».\n` +
    `2. Кейс из ${CASES} — ДВА фильтра сразу: та же отрасль/бизнес-модель клиента И та же механика боли ` +
    `(снапшот прокачан — в браузер не ходить). 🚫 Совпадения одной механики НЕДОСТАТОЧНО: кейса в отрасли нет ` +
    `(АЗС, салоны красоты, подписка в образовании) → заход идёт БЕЗ кейса, вместо него 2–3 наблюдения по модели ` +
    `клиента в стиле Андрея. Подставлять похожее из чужой ниши запрещено.\n` +
